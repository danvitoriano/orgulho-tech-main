...
export default function Footer({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setStatusHtml(
      '<div class="alert alert-info"><span>Enviando cadastro...</span></div>',
    );

    try {
      const response = await fetch("/api/meetup-signup", {
        method: "POST",
        body: formData,
      });
      ...
      setStatusHtml(
        '<div class="alert alert-error"><span>Não foi possível cadastrar seu e-mail agora. Tente novamente.</span></div>',
      );
    } finally {
    ...
    <div className="lg:container mx-auto md:max-w-6xl px-4 pt-16 text-sm">
      <div className="flex flex-col gap-20">
        <div className="flex flex-col gap-6 justify-between lg:flex-row">
          <div>
            <img src={logo.src || ""} width={150} height={28} alt={logo.alt} />
          </div>
            ...
            {links?.map((link) => (
              <div key={link.title}>
                ...
                {link.items?.map((item) => (
                  <a
                    key={item.label}
                    className="block hover:underline link no-underline py-1"
                    href={item.href}
                  >
                    {item.label}
                  </a>
        ...
        <div className="border-primary border-t flex flex-col gap-4 items-center justify-between lg:flex-row lg:items-center py-8">
          <div className="flex flex-col gap-4 items-center lg:flex-row lg:gap-6">
            ...
            <span>{copyright}</span>
            <div className="flex gap-2 justify-between lg:gap-6">
